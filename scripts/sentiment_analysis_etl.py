...
def transform_data(df: pd.DataFrame) -> list:
    ...
    rows = [
        (row['s.content_id'], row['sentiment_score'], row['score_confidence'], row['model_name'], row['analyzed_at'])
...
def store_results(rows: list):
    ...
    print('Loading sentiment analysis results into database...')
...
def run_sentiment_analysis_etl():
